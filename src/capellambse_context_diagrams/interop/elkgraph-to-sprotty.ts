...
export class ElkGraphJsonToSprotty {
  ...
  private isBugged(elkEdge: ElkPrimitiveEdge): Boolean {
    return (
      (elkEdge as any).sections !== undefined &&
      (elkEdge as any).sections.length > 0
    );
  }

  private transferSectionBendpoints(
  ...
  private checkAndRememberId(
    e: ElkGraphElement,
    set: Set<string>,
    generateIdIfRequired: boolean = false
  ) {
    if (e.id === undefined) {
